...
*** Options negotiations (events and actions)
...
In example above the milter might need to add recipients to messages,
but it can forgo adding new headers (to notify, for instance, that the
envelope has been modified) if the MTA doesn't agree on it.

If the milter tries and perform an action that was not negotiated, a
protocol error will be signalled by the MTA.

After the options negotiation phase the context is updated with the
agreed actions/events.  The map entry :events will contain the set of
